...
namespace test {
    ...
    class dataAccessTest {
        ...
        public void testInit() {
            ...
            mockDA = MockUtils.MockDataAccess(DateTime.Now);
        ...
        public void Open_Test() {
            ...
            Assert.AreEqual(ConnectionState.Open, Conn.sqlConnection.State, "Open connection first time. Connection state control");
            ...
            Assert.AreEqual(ConnectionState.Open, Conn.sqlConnection.State, "Open connection second time. Connection state control");
            #endregion
            ...
            Assert.AreEqual(ConnectionState.Open, Conn.sqlConnection.State, "Open connection third time with not persisting. Connection state control");
            ...
            Assert.AreEqual(ConnectionState.Open, Conn.sqlConnection.State, "Open connection with openError. Connection state control");
            Conn.OpenError = false;
            #endregion
            ...
            Assert.AreEqual(ConnectionState.Open, Conn.sqlConnection.State, "Open connection first time with not persisting. Connection state control");
            Conn.Persisting = true;
            #endregion
            ...
            var mockIDataAccess = getMockIDataAccess();
            ...
            Conn.Destroy();
            result = Conn.Open().GetAwaiter().GetResult();
            Assert.IsTrue(result, "Open destroyed connection");
            #endregion
        ...
        public void Close_Test() {
            ...
            Conn.Close();
            Assert.AreEqual(ConnectionState.Closed, Conn.sqlConnection.State);
            #endregion

            #region Chiusura di una connessione persistente / nesting > 0
            Conn.Open();
            Assert.AreEqual(ConnectionState.Open, Conn.sqlConnection.State);
            Conn.Close();
            Assert.AreEqual(ConnectionState.Open, Conn.sqlConnection.State);
        ...
        public void Destroy_Test() {
            Conn.Open();
            Assert.AreEqual(ConnectionState.Open, Conn.sqlConnection.State, "Open connection");

            #region Destroy open connection
            Conn.Destroy();
            Assert.IsNull(Conn.sqlConnection, "Destroy open connection");
            #endregion

            #region Destroy closed connection
            testInit();
            Assert.IsNotNull(Conn.sqlConnection, "Create new MySqlConnection");
            Conn.Open();
            Assert.AreEqual(ConnectionState.Open, Conn.sqlConnection.State, "Open Connection");
            Conn.Persisting = false;
            Conn.Close();
            Assert.AreEqual(ConnectionState.Closed, Conn.sqlConnection.State, "Close Connection");
            Conn.Destroy();
            Assert.IsNull(Conn.sqlConnection, "Destroy open connection");
            #endregion
            ...
            Assert.IsNotNull(Conn.sqlConnection, "Create new MySqlConnection");
            Conn.Destroy();
            Assert.IsNull(Conn.sqlConnection, "Destroy new MySqlConnection");
            Conn.Destroy();
            Assert.IsNull(Conn.sqlConnection, "Simply return because MySqlConnection is null");
            #endregion

            #region Destroy connection with s?.Connection != null
            testInit();
            var result = Conn.BeginTransaction(IsolationLevel.Unspecified);
            Assert.IsNull(result);
            Conn.errorLogger = new Mock<IErrorLogger>(MockBehavior.Default).Object;
            Conn.Destroy();
            Assert.IsNull(Conn.sqlConnection, "Destroy connection with current transaction");
            #endregion
        }
        ...
        public void rDataAccess_Run_Select_Test_Errored() {
			...
			//greater = t._Filter(q.field("idregistryclass") > q.constant("23"));
			Assert.IsNotNull(t, "A table is returned");
        ...
        public void rDataAccess_readValue() {
            //Non buono come test in quanto dipende da un DB reale. Da aggiornare successivamente
            object idreg = Conn.ReadValue("registry", q.eq("cf", "not existent cf !"), "title");            
            Assert.AreEqual(null, idreg, "readValue returns null on no row found");

            object idbadge = Conn.ReadValue("registry", q.eq("idreg", 1), "badgecode");            
        ...
        /// </summary>
        /// <returns></returns>
        public Mock<IDataAccess> getMockIDataAccess() {
            //Creazione di un mockIDataAccess per ottenere una mainConnection diversa da null
    ...
    class mockDataAccessTest {
        ...
        public static void testInit() {
            mDA = new Mock<DataAccess>("dsn", "dummyServer", "dummyDataBase", "user", "password", DateTime.Now.Year, DateTime.Now.Date);
            Conn = mDA.Object;
            //QHS = Conn.GetQueryHelper();
        }
        [OneTimeTearDown]
        ...
        public void setMockSecurity(ref Mock<ISecurity> sec, string name = null, string response = null) {
        ...
        public void mDataAccess_RUN_SELECT() {
            ...
            var a = Conn.Select("config");
            Assert.AreEqual(t1, a, "RUN SELECT table named config");

            a = Conn.Select("config", "*", "test", "test1", "test1");
            Assert.AreEqual(t1, a, "RUN SELECT table named config with more parameters not null");

            Assert.AreEqual(null, Conn.Select("registry"), "RUN SELECT with inesistent table");
        }

        [Test]
        public void mDataAccess_DO_DELETE() {
            //Non metto alcun return in quanto se tutto è andato a buon fine fa return null
            mDA.Setup(x => x.DoDelete(It.IsAny<string>(), It.IsAny<string>()));

            var temp = Conn.DoDelete("dummyTable", "dummyCondition");
            Assert.IsNull(temp, "DO_DELETE with table and condition");
        }
        ...
        public void mDataAccess_Commit() {
            ...
            Assert.IsNull(Conn.Commit(), "Commit mock is ok");
        }

        [Test]
        public void mDataAccess_DO_INSERT() {
            //Non metto alcun return in quanto se tutto è andato a buon fine fa return null
            mDA.Setup(x => x.DoInsert(It.IsAny<string>(), It.IsAny<List<string>>(),It.IsAny<List<object>>()));

            var temp = Conn.DoInsert("dummyTable", new List<string>(){ "a", "b" }, new List<object>() { "1", "2" });
            Assert.IsNull(temp, "DO_INSERT with table");
        }

        [Test]
        public void mDataAccess_CreateTableByName() {
            mDA.Setup(x => x.CreateTable(It.IsAny<string>(), 
	            It.IsAny<string>(),It.IsAny<bool>()))
	            .Returns(async (string name, string nameList) => { return new DataTable(name); });
