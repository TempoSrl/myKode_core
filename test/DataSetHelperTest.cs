...
namespace test {
   [TestFixture]
    public class DataSetHelperTest {
        ...
        [Test]
        public async void JoinTest2() {
            DataTable t = Conn.Select(tablename: "mandatedetail", filter:QHS.CmpGe("tax", 10), top:"100").GetAwaiter().GetResult();
        ...
        public void selectFilterCompiledTest() {
            ...
            Assert.IsTrue(s0.ElapsedMilliseconds > s00.ElapsedMilliseconds, 
	            $"Select is slower than code ({s0.ElapsedMilliseconds} > {s00.ElapsedMilliseconds})");

            Assert.IsTrue(s1.ElapsedMilliseconds > s0.ElapsedMilliseconds, 
	            $"Filter is slower than Select ({s1.ElapsedMilliseconds} > {s0.ElapsedMilliseconds})");

            Assert.IsTrue(s2.ElapsedMilliseconds < s0.ElapsedMilliseconds, 
	            $"Compiled Filter is faster than Select ({s2.ElapsedMilliseconds} < {s0.ElapsedMilliseconds})");


            Assert.IsTrue(s2.ElapsedMilliseconds*2 < s1.ElapsedMilliseconds, 
                    $"Compiled Filter is quicker than Filter ({s2.ElapsedMilliseconds} < {s1.ElapsedMilliseconds})");

            Assert.IsTrue(s3.ElapsedMilliseconds < s2.ElapsedMilliseconds, 
	            $"Compiled Filter is slower than code ({s3.ElapsedMilliseconds} < {s2.ElapsedMilliseconds})");
        ...
        public void ObjectSelectTest() {
            var t = Conn.RowObjectSelect("mandatedetail", filter:QHS.CmpGe("tax", 10), TOP:"100").GetAwaiter().GetResult();
        ...
        public void TestSelectSpeed() {
            ...
            var t0 = Conn.RowObjectSelect("mandatedetail", filter:QHS.AppAnd(QHS.CmpGe("tax", 0),QHS.IsNotNull("taxable")), TOP:"10").GetAwaiter().GetResult();

            Stopwatch select1 = new Stopwatch();
            select1.Start();
            var t1 = Conn.RowObjectSelect("mandatedetail",filter:  QHS.AppAnd(QHS.CmpGe("tax", 0),QHS.IsNotNull("taxable")), TOP: $"{limit}").GetAwaiter().GetResult();
            select1.Stop();
