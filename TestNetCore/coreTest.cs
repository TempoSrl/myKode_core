   ...
   class sampleBaseClass {
    ...
    public class DotNetTest {          
        ...
        public void callerMethod1() {
            ...
            Assert.AreEqual(27, result, "test call with named params");
        }
    ...
    public class CoreTest {
        ...
        public void CreateTableByName() {
            DataTable t =  Conn.CreateTable("expense",addExtProp:true).GetAwaiter().GetResult();
        ...
        public void runSelectSpeed1000_by_10() {
            ...
            for (int i = 0; i < nIteration; i++) {
                var t4 =  Conn.ReadTableNoKey("entry", TOP:top).GetAwaiter().GetResult();
            }
            SreadFromTable.Stop();
            ...
            for (int i = 0; i < nIteration; i++) {
                var t5 = Conn.SelectRowObjects("entry",TOP:top).GetAwaiter().GetResult();
            }
            SRowObject_Select.Stop();

            Console.WriteLine($"Rows read:{rCount}");
            Console.WriteLine($"Select            duration:{SRUN_SELECT.ElapsedMilliseconds}");
            Console.WriteLine($"ReadTableNoKey    duration:{SreadFromTable.ElapsedMilliseconds}");
            Console.WriteLine($"SelectRowObjects  duration:{SRowObject_Select.ElapsedMilliseconds}");
            Assert.IsTrue(SreadFromTable.ElapsedMilliseconds > SRowObject_Select.ElapsedMilliseconds, 
                $"SelectRowObjects ({SRowObject_Select.ElapsedMilliseconds})  is quicker than ReadTableNoKey ({SreadFromTable.ElapsedMilliseconds})");
        ...
        public void runSelectSpeed10_by_10000() {
            ...
            SRowObject_Select.Start();
            for (int i = 0; i < nIteration; i++) {
                var t5 = Conn.SelectRowObjects("entry", TOP: top);
            }
            SRowObject_Select.Stop();
            ...
            Assert.IsTrue(SRUN_SELECT.ElapsedMilliseconds > SreadFromTable.ElapsedMilliseconds,
                $"readFromTable  is  quicker  than runSelect ({SRUN_SELECT.ElapsedMilliseconds} > {SreadFromTable.ElapsedMilliseconds}) ");
            Assert.IsTrue(SRowObject_Select.ElapsedMilliseconds < SreadFromTable.ElapsedMilliseconds, 
                $"RowObject_Select  is quicker than readFromTable ({SRowObject_Select.ElapsedMilliseconds} < {SreadFromTable.ElapsedMilliseconds}) ");
