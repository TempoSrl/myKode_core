   ...
   [TestFixture]
    public class MetaExpressionTest {
       ...
       [Test]
        ...
        [Test]
        public void sys_test_Dictionary() {
            q m1 = q.sys("a");
            q m2 = q.sys("b");
            var env = new DefaultSecurity("user");
            env.SetSys("a","1");
            env.SetSys("b", 2);

            Assert.AreEqual("1", m1.apply(null,env));
            Assert.AreEqual(2, m2.apply(null,env));
        }
        ...
        public void isConditionalNull_test() {
            ...
            // -------------------------------------------------------------------------------
            // Arrange 
            var r1 = new TestNetCore.normalClassNullable() { c = 1 };
            var r2 = new TestNetCore.normalClassNullable();
            var m1 = q.isNull("a");
            var m2 = q.isNull("c");
        ...
        public void collectionTest() {
            ...
            #region Predicate
            //this is only to check if syntax is allowed
            q filter =q.ne("c",1);
            var tempColl0 = coll2 as object[];
            var filteredColl0 = tempColl0.Where(filter);
